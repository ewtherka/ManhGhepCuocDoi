...
class mainBoard(Match3Board):
    ...
    def _special_block_spawn(self, matched_group):
        ...
        for match_line in matched_group:
            ...
            for col, row in match_line:
                ...
                if (
                    block != self.empty
                ):  # phòng TH block đã bị delete khỏi mảng (gọi sau clear)
    ...
    def populate(
        ...
        no_valid_play_check: bool = True,
        no_match3_group_check: bool = True,
        _retry_count: int = 0,
    ) -> list[tuple[int, int]]:
        """
            ...
            for col in range(cols[0], cols[1]):
                if self.board[row][col] == self.empty:
                    ...
                    while len(values_left) > 0:
                        ...
                        if not no_match3_group_check or not self.filter_group(
                            self.get_group(col, row)
                        ):
                            break

                    # chống crash recursionError
                    # nếu tạo thành match-3 thì tạo lại (quá 10 lần thì buông xuôi để nó match)
                    if no_match3_group_check and len(values_left) == 0:
                        self.board = backup_board
                        if _retry_count > 10:
                            return self.populate(
                                cols, rows, no_valid_play_check, False, 0
                            )
                        else:
                            return self.populate(
                                cols,
                                rows,
                                no_valid_play_check,
                                no_match3_group_check,
                                _retry_count + 1,
                            )

                    populated.append((col, row))

        # nếu không tạo thành nước đi nào được thì tạo lại (quá 10 lần thì trả về [] để tạo bảng mới)
        if no_valid_play_check and len(self.find_a_play()) == 0:
            self.board = backup_board
            if _retry_count > 10:
                return []
            return self.populate(
                cols, rows, no_valid_play_check, no_match3_group_check, _retry_count + 1
            )
    ...
    def swap(self, point1: tuple[int, int], point2: tuple[int, int]) -> None:
        ...
        # phòng khi phải code khối băng rơi thay vì đứng yên
        (x1, y1), (x2, y2) = point1, point2
        self.ice_board[y1][x1], self.ice_board[y2][x2] = (
            self.ice_board[y2][x2],
            self.ice_board[y1][x1],
        )

    def get_group(
        self, col: int, row: int, group: list[tuple[int, int]] = None
    ) -> list[tuple[int, int]]:
        """
        ...
        for offset_x, offset_y in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            ...
            if (
                self.board[neigh_y][neigh_x] == self.FATE
                or self.ice_board[neigh_y][neigh_x] > 0
            ):
                continue

            if self._get_block_type(self.board[row][col]) != self._get_block_type(
                self.board[neigh_y][neigh_x]
            ):
                continue
    ...
    def is_swap_valid(self, point1: tuple[int, int], point2: tuple[int, int]) -> bool:
        """
        Check nước đi hợp lệ (trừ trường hợp swap khối ngoài bảng chơi và khối đóng băng).
        """
    ...
    def find_a_play(
        self,
    ) -> tuple[tuple[tuple[int, int], tuple[int, int]], list[list[tuple[int, int]]]]:
        """
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if (
                    self.board[row][col] == self.FATE
                ):  # nếu có FATE thì ấn vào được, không cần tìm nước đi nữa
                ...
                for x, y in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                    ...
                    if (
                        self.out_of_bounds(neigh_x, neigh_y)
                        or self.ice_board[neigh_y][neigh_x] > 0
                    ):
                        continue

                    if self._get_block_type(
                        self.board[row][col]
                    ) == self._get_block_type(self.board[neigh_y][neigh_x]):
                        continue
                ...
                for col in range(self.cols):
                    if self.board[row + 1][col] == self.empty:
                        ...
                        if (
                            ...
                            and self.ice_board[row][col] == 0
                        ):
                            self.swap((col, row), (col, row + 1))
                            initial_positions[(col, row + 1)] = initial_positions.pop(
                                (col, row)
                            )
                            moved = True
                            continue
                        ...
                        if self.ice_board[row][col] > 0:
                            ...
                            for r in range(
                                row - 1, -1, -1
                            ):  # check khoảng trống dưới khối băng
                                if self.ice_board[r][col] > 0:
                                    is_shadowed = True
                                    break
                        ...
                        if is_shadowed:
                            ...
                            for dx in dirs:
                                nx = col + dx
                                if (
                                    not self.out_of_bounds(nx, row)
                                    and self.board[row][nx] != self.empty
                                    and self.ice_board[row][nx] == 0
                                ):
                                    if (
                                        self.board[row + 1][nx] != self.empty
                                        or self.ice_board[row + 1][nx] > 0
                                    ):
                                        self.swap((nx, row), (col, row + 1))
                                        initial_positions[(col, row + 1)] = (
                                            initial_positions.pop((nx, row))
                                        )
                                        moved = True
                                        break
        # trả về dict điểm đầu - cuối để GUI làm animation (nếu khối đứng yên thì không thêm vào dict)
