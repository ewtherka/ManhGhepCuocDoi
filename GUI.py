...
class mainGUI(Match3GUI):
    ...
    """
    preferences_schema = json.loads(
        preferences_schema
    )  # chuyen string trong file json thanh dict de quan li pref
    ...
    def __init__(self):
        ...
        self.hobby_icons = [
            ...
            [None, None, None],
            [None, None, None],
        ]  # 3 type 4 state (state 0 o trne roi)
        self.hobby_boost_icons = [
            [None, None, None, None],
            [None, None, None, None],
            [None, None, None, None],
        ]  # 3 type 4 state
    ...
    def draw_tile(self, x, y, color_index, size=None, ice_hp=0):
        ...
        # BOOST_CHANCE: icon boost theo type, fallback về icon boost chung
            ...
            if self.chance_type is not None:
                icon = (
                    self.chance_boost_icons[self.chance_type] or self.chance_boost_icon
                )
            else:
        ...
        if icon is not None and diameter > 0:
            ...
            )  # diameter full khi nằm trên ô
            # kích thước đầy đủ: dùng cache để không smoothscale lại mỗi frame
            if diameter == full_d:
                key = (
                    id(icon),
                    diameter,
                )  # mot bo gom dia chi vung nho chua anh va diameter cua anh
                if key not in self._icon_cache:
                    self._icon_cache[key] = pygame.transform.smoothscale(
            ...
            if ice_hp > 0:
                ...
                if ice_hp == 2:
                    pygame.draw.rect(
                        ice_surf, ice_color, ice_surf.get_rect(), border_radius=8
                    )
                    pygame.draw.rect(
                        ice_surf,
                        (255, 255, 255, 100),
                    ...
                    half_rect = pygame.Rect(
                        0, ice_diameter // 2, ice_diameter, ice_diameter // 2
                    )
                    pygame.draw.rect(
                        ice_surf,
                        ice_color,
                ...
                self.board_surf.blit(
                    ice_surf, (x - ice_diameter // 2, y - ice_diameter // 2)
                )
            return
    ...
    def _draw_button(
        ...
        if text and fd:
            lbl = fd.render(
                text, True, self.widget_text_color
            )  # chuyen text sang img de tao label
            ...
            )  # phải cộng thêm offset cho y vì nút có hình dạng đặc biệt
    ...
    def _chance_dialog_icon(self):
        ...
        return (
            self.chance_boost_icons[self.chance_type]
            or self.chance_icons[self.chance_type]
            or self.chance_boost_icon
            or self.chance_icon_s0
        )
    ...
    def draw_dialog(self, content, icons=None):
        ...
        box_diag = (
            math.sqrt(gw**2 + gh**2) * 0.52
        )  # tính đường chéo bằng pytago, sau đó nhân với 52% để dialogue bõ k bị nhỏ quá
        box_w = int(
            box_diag * 16 / math.sqrt(16**2 + 9**2)
        )  # lấy (đường chéo hiện tại / đường chéo chuẩn 16:9).cạnh
        ...
        if self.dialogue_box_img is not None:
            ...
            pygame.draw.rect(
                self.game_surf, (180, 168, 155), rect, width=2, border_radius=10
            )

        # giới hạn vùng vẽ vào content_rect để chữ không tràn ra ngoài padding, lùi sang bên phải/dưới một khoảng pad
        content_rect = pygame.Rect(
            rect.left + pad, rect.top + pad_v, box_w - pad * 2, box_h - pad_v * 2
        )
        ...
        if isinstance(content, dict) and "quote" in content:
            ...
            total_h = (
                icon_row_h
                + len(header_lines) * lh_large
                + sep
                + len(quote_lines) * lh
                + sep
                + len(speaker_lines) * lh
                + sep
            ...
            for line in speaker_lines:
                blit_line(
                    fds, line, COL_SPEAKER, x_center=False, x_right=content_rect.right
                )
            ty += sep
            ...
            lines = (
                content.get("lines", content) if isinstance(content, dict) else content
            )
            header_lines = self._wrap_text(header, fdl, max_text_w) if header else []
            all_lines = []
            for line in lines:
    ...
    def draw_board(self, no_draw_pts=None):
        ...
        if self.board_area_bg_scaled is not None:
            self.game_surf.blit(
                self.board_area_bg_scaled, (0, 0)
            )  # bỏ background cho game board vào
        else:
            self.board_surf.fill(
                self.background_color["board"]
            )  # fallback fill solid color
    ...
    def draw_buttons(self, texts, y, y_separation, surface_name):
        ...
        for text in texts:
            ...
            self._draw_button(
                ...
                text,
                button_name,
                getattr(self, f"{button_name}_clicked"),
    ...
    def draw_sidebar(self):
        ...
        self.sidebar_surf.blit(
            stage_lbl,
            (tf_x + (tf_w - stage_lbl.get_width()) // 2, tf_top + int(tf_h * 0.20)),
        )
        ...
        hg_img = (
            self.hourglass_imgs[hg_idx] if hg_idx < len(self.hourglass_imgs) else None
        )
    ...
    def draw_ended(self):
        ...
        y = (
            self.game_surf.get_height() - (self.char_height + self.char_sep_height) * 6
        ) / 2  # canh giữa, gap 1.5, continue 3.5 còn game over 1
    ...
    def draw_preferences(self):
        self.game_surf.fill(self.background_color["game"])
        y = (
            self.game_surf.get_height() - (self.char_height + self.char_sep_height) * 12
        ) / 2
    ...
    def resize_surfaces(self):
        ...
        self.game_surf = self.screen_surf.subsurface(
            (0, 0, gw, gh)
        )  # canvas game = canvas màn
        ...
        self.board_surf = self.game_surf.subsurface(
            (0, board_top, board_side, board_side)
        )  # vẽ từ 0->board size và từ top -> boardsize cho canvas board

        if self.board is not None:
            self.circle_radius = board_side / (
                self.board.cols * 2
            )  # bán kính = (boardsize/số cột(hàng))/2

        self.sidebar_surf = self.game_surf.subsurface(
            (left_w, 0, gw - left_w, gh)
        )  # canvas sideboard từ left->hết và 0->hết

        if self.sidebar_bg is not None:
            self.sidebar_bg_scaled = pygame.transform.smoothscale(
                self.sidebar_bg, (gw - left_w, gh)
            )  # actually board bg là của sizebar...
    ...
    def animate_swap(self, board_point1, board_point2):
        ...
        win_points = (
            list(self.board_pos_to_win_pos(*board_points[0])),
        ...
        while curr_pos[0] != win_points[1] or curr_pos[1] != win_points[0]:
            if self.process_events():
                ...
                target_dist = (
                    ...
                    [
                        win_points[0][0] - win_points[1][0],
                        win_points[0][1] - win_points[1][1],
                    ],
                )
            ...
            for p_i in reversed(range(2)):
                ...
                curr_dist = (
                    target_dist[p_i][0] * curr_ani_time / self.swap_ani_time,
                ...
                for i in range(2):
                    ...
                    if (dir < 0 and curr_pos[p_i][i] < dst_pos[i]) or (
                        dir > 0 and curr_pos[p_i][i] > dst_pos[i]
                    ):
                        curr_pos[p_i][i] = dst_pos[i]
                color_index = self.board.board[board_points[p_i][1]][
                    board_points[p_i][0]
                ]
                if color_index < 0:
                    continue
                ice_hp = (
                    self.board.ice_board[board_points[p_i][1]][board_points[p_i][0]]
                    if hasattr(self.board, "ice_board")
                    else 0
                )
                self.draw_tile(
                    curr_pos[p_i][0], curr_pos[p_i][1], color_index, ice_hp=ice_hp
                )
            pygame.display.flip()
    ...
    def animate_clear(self, board_points, no_more_moves=False):
        ...
        while curr_transparency != 0 or curr_size != 0:
            ...
            if no_more_moves:
                ...
                height = (
                    math.ceil(self.char_height) + math.ceil(self.char_sep_height)
                ) * 2
                x = (
                    self.board_surf.get_width() - width
                ) / 2 + self.board_surf.get_abs_offset()[0]
                y = (
                    self.board_surf.get_height() - height * 2
                ) / 2 + self.board_surf.get_abs_offset()[1]
                for text in texts:
                    btn = pygamew.Button(
                        self.screen_surf,
                        x,
    ...
    def animate_shift_down(self, shifted_bp, num_vertical_points, src_pts=None):
        ...
        board_points_src = (
            ...
            else [(x, y - 1) for (x, y) in board_points_dst]
        )
        ...
        while any([curr_pos[i] != win_points_dst[i] for i in range(len(curr_pos))]):
            if self.process_events():
                ...
                self.draw_sidebar()
                win_points_dst = [
                    list(self.board_pos_to_win_pos(*p)) for p in board_points_dst
                ]
                win_points_src = [
                    list(self.board_pos_to_win_pos(*p)) for p in board_points_src
                ]
            ...
            for p_i in range(len(curr_pos)):
                ...
                curr_dist = (
                    target_dist[0] * curr_ani_time / ani_time,
                    target_dist[1] * curr_ani_time / ani_time,
                )
                curr_pos[p_i] = [src_pos[0] + curr_dist[0], src_pos[1] + curr_dist[1]]
                curr_pos[p_i] = [int(curr_pos[p_i][0]), int(curr_pos[p_i][1])]
                for i in range(2):
                ...
                ice_hp = (
                    self.board.ice_board[dst_y][dst_x]
                    if hasattr(self.board, "ice_board")
                    else 0
                )
                self.draw_tile(
    ...
    def animate_hint(self, board_point1, board_point2):
        ...
        while True:
            ...
            shake_y = int(
                math.sin(t * FREQ * 2 * math.pi * 1.3 + 1.0) * AMPLITUDE * 0.4 * decay
            )
            for p_i in range(2):
                color_index = self.board.board[board_points[p_i][1]][
                    board_points[p_i][0]
    ...
    def endings(self, call_type):
        if call_type == 0:
            self.show_dialog_and_wait(
                self.DIALOG_LINES[call_type][self.hobby_state][self.hobby_type],
                icons=[self._hobby_dialog_icon()],
            )
        elif call_type == 1:
            self.show_dialog_and_wait(
                self.DIALOG_LINES[call_type][self.random_wheel_result][
                    self.chance_type
                ],
            ...
            if self.hobby_count >= 50:
                self.show_dialog_and_wait(
                    self.DIALOG_LINES[call_type][self.hobby_type],
                    icons=[self._hobby_dialog_icon()],
                )
            elif self.chance_count >= 48:
                self.show_dialog_and_wait(
                    self.DIALOG_LINES[call_type][self.chance_type],
                    icons=[self._chance_dialog_icon()],
                )
            else:
                lost_icons = [
                    i
                    for i in [self._chance_dialog_icon(), self._hobby_dialog_icon()]
                    if i
                ]
                self.show_dialog_and_wait(
                    self.DIALOG_LINES[call_type][4], icons=lost_icons or None
                )
    ...
    def _apply_fate_event(self):
        ...
        event = random.choice(
            ["philosopher", "seer", "sage", "thief", "brute", "prisoner"]
        )
        if event == "philosopher":
            self.philosopher_turns = 2
        elif event == "sage":
            self.sage_turns = 2
        elif event == "prisoner":
            self.prisoner_turns = 2
        dialog_key = f"fate_{event}"
        if dialog_key in self.DIALOG_LINES:
            self.show_dialog_and_wait(
                self.DIALOG_LINES[dialog_key],
        ...
        while True:
            ...
            populated = self.board.populate(
                rows=(0, self.board.rows),
                no_valid_play_check=False,
                no_match3_group_check=True,
            )

            if not movements and not populated:
                break
            ...
            while True:
                ...
                if anim_dst:
                    self.animate_shift_down(
                        anim_dst,
                        self.get_num_vertical_points(chance_pts),
                        src_pts=anim_src,
                    )
                self.play_sound("drop")
                self.update_board()
    ...
    def minigame(self):
        ...
        if self.hobby_type == 0:
            result = minigame.Tailor(
                self.board_surf, self.clock, difficulty
            ).run()  # KVan sua
        ...
        if result:
            ...
            if "hobby_state_3" in self.DIALOG_LINES and self.hobby_type is not None:
                self.show_dialog_and_wait(
                    self.DIALOG_LINES["hobby_state_3"][self.hobby_type],
                    icons=[self._hobby_dialog_icon()],
                )
    ...
    def check_stat_thresholds(self, check_time=True):
        ...
        if new_hobby > self.hobby_state and self.hobby_state != 3:
            ...
            if key in self.DIALOG_LINES and self.hobby_type is not None:
                self.show_dialog_and_wait(
                    self.DIALOG_LINES[key][self.hobby_type],
                    icons=[self._hobby_dialog_icon()],
                )
        ...
        if new_chance > self.chance_state:
            ...
            if self.chance_state == 1 and self.chance_type is not None:
                self.show_dialog_and_wait(
                    self.DIALOG_LINES["chance_state_1"][self.chance_type],
                    icons=[self._chance_dialog_icon()],
                )
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.mouse_state == MouseState.WAITING:
                    ...
                    if self.point_inside_circle(
                        event.pos, circle_center, self.circle_radius * self.circle_scale
                    ):
                ...
                if self.mouse_state == MouseState.MOVING:
                    ...
                    for x, y in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                        if [
                            self.board_pos_src[0] + x,
                            self.board_pos_src[1] + y,
                        ] == board_pos_dst:
                            swap_valid = True
                            break
                    if not swap_valid:
                    ...
                    sx, sy = self.board_pos_src
                    dx, dy = board_pos_dst
                    if hasattr(self.board, "ice_board") and (
                        self.board.ice_board[sy][sx] > 0
                        or self.board.ice_board[dy][dx] > 0
                    ):
                        self.mouse_state = MouseState.WAITING
                        continue

                    swap_valid = self.board.is_swap_valid(
                        self.board_pos_src, board_pos_dst
                    )
    ...
    def running(self):
        ...
        while len(groups) > 0:
            ...
            for cx, cy in points:
                ...
                if bv == mainBoard.TIME:
                    time_raw += 1
                elif bv == mainBoard.BOOST_TIME:
                    time_raw += 1
                    has_boost_time = True
                elif bv == mainBoard.HOBBY:
                    hobby_raw += 1
                elif bv == mainBoard.BOOST_HOBBY:
                    hobby_raw += 1
                    has_boost_hobby = True
                elif bv == mainBoard.CHANCE:
                    chance_raw += 1
                elif bv == mainBoard.BOOST_CHANCE:
                    chance_raw += 1
                    has_boost_chance = True
            time_mult = 2 if self.prisoner_turns > 0 else 1
            hobby_mult = 2 if self.sage_turns > 0 else 1
            time_gain = (
                0
                if self.philosopher_turns > 0
                else int(time_raw * (1.5 if has_boost_time else 1)) * time_mult
            )
            time_pts = [
                (c, r)
                for (c, r) in points
            ...
            self.hobby_count = min(
                self.hobby_count
                + int(hobby_raw * (1.5 if has_boost_hobby else 1)) * hobby_mult,
                60,
            )
            ...
            for pos, btype in spawns:
                sx, sy = pos
                if (
                    not self.board.out_of_bounds(sx, sy)
                    and self.board.board[sy][sx] == self.board.empty
                ):
            ...
            while True:
                ...
                if anim_dst:
                    self.animate_shift_down(
                        anim_dst, self.get_num_vertical_points(points), src_pts=anim_src
                    )
                self.play_sound("drop")
                self.update_board()
        ...
        if getattr(self, "turn_taken", False):  # KVan sua
            if self.philosopher_turns > 0:
                self.philosopher_turns -= 1
            if self.sage_turns > 0:
                self.sage_turns -= 1
            if self.prisoner_turns > 0:
                self.prisoner_turns -= 1
            self.turn_taken = False  # KVan sua
    ...
    def load_icon(self):
        ...
        if os.path.isfile("media/images/ui/backgroundA.png"):
            self.sidebar_bg = pygame.image.load(
                "media/images/ui/backgroundA.png"
            ).convert()
        if os.path.isfile("media/images/ui/backgroundB.png"):
            self.board_area_bg = pygame.image.load(
                "media/images/ui/backgroundB.png"
            ).convert()
        ...
        if os.path.isfile("media/images/block/hobby_0.png"):
            self.hobby_icon_s0 = pygame.image.load(
                "media/images/block/hobby_0.png"
            ).convert_alpha()
        ...
        for t, tname in enumerate(hobby_types):
            ...
            for s, stage in enumerate(["1", "2", "3"]):  # KVan sua
                ...
                if os.path.isfile(p):
                    self.hobby_boost_icons[t][s + 1] = pygame.image.load(
                        p
                    ).convert_alpha()
        if os.path.isfile("media/images/block/hobby_boost.png"):
            generic = pygame.image.load(
                "media/images/block/hobby_boost.png"
            ).convert_alpha()
            for t in range(3):
        ...
        if os.path.isfile("media/images/block/chance_0.png"):
            self.chance_icon_s0 = pygame.image.load(
                "media/images/block/chance_0.png"
            ).convert_alpha()
        ...
        if os.path.isfile("media/images/block/chance_boost.png"):
            self.chance_boost_icon = pygame.image.load(
                "media/images/block/chance_boost.png"
            ).convert_alpha()
        for t, tname in enumerate(chance_types):
        ...
        for i, path in enumerate(
            ["media/images/block/time_0.png", "media/images/block/time_boost.png"]
        ):
            if os.path.isfile(path):
        ...
        if os.path.isfile("media/images/block/fate.png"):
            self.fate_icon = pygame.image.load(
                "media/images/block/fate.png"
            ).convert_alpha()
            ...
            with open(self.preferences_filename, "r") as file:
                try:
                    ...
                    try:
                        ...
                        print(
                            f"ERROR: In file {self.preferences_filename}: json doesn't conform to schema."
                        )
                except json.JSONDecodeError:
                    print(
                        f"ERROR: In file {self.preferences_filename}: json not valid."
                    )
